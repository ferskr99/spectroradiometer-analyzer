  ...
  return (
    <div style={styles.tooltip}>
      ...
      <div style={{ maxHeight: 200, overflowY: "auto", paddingRight: 4 }}>
        {payload.map((entry, i) => (
          <p key={`${entry.name}-${i}`} style={{ ...styles.tooltipValue, color: entry.color }}>
...
export const SuperimposedGraph: React.FC<SuperimposedGraphProps> = ({
  ...
  const [showRaw711G, setShowRaw711G] = useState(false);
  const [showRaw713G, setShowRaw713G] = useState(false);
  const [showRaw711D, setShowRaw711D] = useState(false);
  const [showRaw713D, setShowRaw713D] = useState(false);
