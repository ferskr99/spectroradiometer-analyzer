...
export type SensorId = "MS-711" | "MS-713" | "MS-711_GLOBAL" | "MS-713_GLOBAL" | "MS-711_DIRECT" | "MS-713_DIRECT";
...
export type AnalysisResult = components["schemas"]["AnalysisResult"] & {
  ...
  applied_exposure_per_sensor?: Record<string, number> | null;
  global_spectrum?: SpectralData | null;
  direct_spectrum?: SpectralData | null;
  diffuse_spectrum?: SpectralData | null;
  dni?: number | null;
  dhi?: number | null;
  clearness_index?: number | null;
  diffuse_fraction?: number | null;
  cross_metrics?: Record<string, any> | null;
  raw_spectra?: Record<string, SpectralData> | null;
};
export type HTTPValidationError = components["schemas"]["HTTPValidationError"];

export interface AnalysisRequest {
  sensor_target: "Global" | "Direct" | "All" | string;
...
export class SpectroradiometerApiClient {
   ...
   */
  async getHistory(page: number = 1, limit: number = 50): Promise<{
    ...
    return this.request(
      `/api/v1/sensors/history/list?page=${page}&limit=${limit}`,
      { method: "GET" },
    );
  }

  /**
   * GET /api/v1/sensors/history/timeseries
   *
   * Obtiene métricas radiométricas en un rango de fechas.
   */
  async getHistoryTimeSeries(startDate: string, endDate: string): Promise<{
    points: {
      timestamp: string;
      par: number | null;
      pwv: number | null;
    ...
    total_irradiation_mj: number;
  }> {
    return this.request(
      `/api/v1/sensors/history/timeseries?start_date=${encodeURIComponent(startDate)}&end_date=${encodeURIComponent(endDate)}`,
      { method: "GET" }
    );
  }
  ...
  /**
   * GET /api/v1/sensors/settings/hardware
   * Lee la configuración de puertos del hardware.
   */
  async getHardwareSettings(): Promise<HardwareSettings> {
    return this.request<HardwareSettings>(
      `/api/v1/sensors/settings/hardware`,
      { method: "GET" }
    );
  }

  /**
   * POST /api/v1/sensors/settings/hardware
   * Guarda la configuración de puertos del hardware.
   */
  async saveHardwareSettings(config: HardwareSettings): Promise<any> {
    return this.request<any>(
      `/api/v1/sensors/settings/hardware`,
      {
        method: "POST",
  ...
  /**
   * GET /api/v1/sensors/settings/ports
   * Lista los puertos serie disponibles en el sistema.
   */
  async listSerialPorts(): Promise<{ ports: SerialPortInfo[]; warning?: string }> {
    return this.request<{ ports: SerialPortInfo[]; warning?: string }>(
      `/api/v1/sensors/settings/ports`,
      { method: "GET" }
    );
  }

  async testConnection(port: string): Promise<any> {
    return this.request<any>(
      `/api/v1/sensors/settings/test-connection`,
      {
        method: "POST",
  ...
  // ─── Sun Tracker Calibración ──────────────────────────────────────

  /**
   * POST /api/v1/sensors/tracker/calibrate
   * Envía un comando de calibración al Sun Tracker (STR-22G/32G).
   */
  async calibrateTracker(payload: TrackerCalibrationCommand): Promise<any> {
    return this.request<any>(
      `/api/v1/sensors/tracker/calibrate`,
      {
        method: "POST",
  ...
  /**
   * GET /api/v1/sensors/tracker/location
   * Obtiene la ubicación GPS configurada en el tracker.
   */
  async getTrackerLocation(): Promise<{ status: string; location: { latitude: number; longitude: number; elevation: number } | null }> {
    return this.request<any>(
      `/api/v1/sensors/tracker/location`,
      { method: "GET" }
    );
  }
...
export type TrackerCalibrationCommand =
  | { command: "SYNC_TIME" }
  | { command: "SET_LOCATION"; latitude: number; longitude: number; elevation?: number }
  | { command: "AUTO_ALIGN" };
